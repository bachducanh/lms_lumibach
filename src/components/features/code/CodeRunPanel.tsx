      ...
      <div className="flex items-center gap-2 border-b border-border bg-muted/40 px-3 py-2">
        ...
        <div className="relative">
          <select
            ...
            className="appearance-none rounded-md border border-input bg-background pl-3 pr-7 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-ring disabled:opacity-60 cursor-pointer"
          >
            {LANGUAGES.map((l) => (
        ...
        <div className="relative">
          <select
            ...
            className="appearance-none rounded-md border border-input bg-background pl-3 pr-7 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-ring cursor-pointer"
          >
        ...
        <div className="flex w-72 shrink-0 flex-col">
          ...
          <div className="flex flex-1 flex-col overflow-hidden">
            <div className="flex items-center justify-between px-3 py-1.5">
              ...
              {result?.success && (
                <span className={cn('text-xs font-semibold', STATUS_CLASS[result.status.id] ?? 'text-muted-foreground')}>
            ...
            <div className="flex-1 overflow-auto px-3 pb-3">
              ...
              {!running && output && (
                <pre className={cn(
                  ...
                  {output.text || (
                    <span className="italic text-muted-foreground/50">(không có output)</span>
                  )}
                </pre>
              )}
              {!running && !output && (
                <p className="text-xs italic text-muted-foreground/50">Nhấn Chạy để xem kết quả</p>
              )}
            </div>
