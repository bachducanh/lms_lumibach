...
export function TestCaseBuilder({ initial, onChange }: Props) {
  ...
  const inputCls = 'w-full rounded-md border border-input bg-background px-2 py-1.5 font-mono text-xs focus:outline-none focus:ring-1 focus:ring-ring resize-none';

  return (
    <div className="space-y-3">
      {/* Header */}
      <div className="grid grid-cols-[2fr_2fr_5rem_4rem_3rem_2rem] gap-2 border-b border-border pb-1">
      ...
      {tcs.map((tc, i) => (
        <div key={i} className="grid grid-cols-[2fr_2fr_5rem_4rem_3rem_2rem] gap-2 items-start">
          <textarea
            value={tc.input}
          ...
          <input
            ...
            className="w-full rounded-md border border-input bg-background px-2 py-1.5 text-xs text-center focus:outline-none focus:ring-1 focus:ring-ring"
          />
          ...
          <button
            ...
            className={cn(
              'mt-1 flex h-6 w-6 items-center justify-center rounded text-muted-foreground transition-colors',
      ...
      <p className="text-xs text-muted-foreground">
        <span className="font-medium">Ẩn ☑</span> = học sinh không thấy input/output. Chạy mẫu chỉ dùng test không ẩn.
      </p>
    </div>
  );
}
