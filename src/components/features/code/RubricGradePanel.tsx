    ...
    <div className="rounded-xl border border-border/60 bg-card/40 p-4 space-y-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="text-xs text-muted-foreground">
          Tổng <span className="font-bold text-foreground tabular-nums">{total}</span>
      ...
      <ul className="space-y-3">
        {rubric.criteria.map((c) => {
          ...
          return (
            <li key={c.id} className="rounded-lg border border-border/60 bg-background/60 p-3 space-y-2">
              <div className="flex items-center justify-between gap-2">
                <p className="text-sm font-medium">{c.name}</p>
                <span className="text-[11px] text-muted-foreground shrink-0">tối đa {max} đ</span>
              </div>
              {c.description && (
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between gap-1">
                        <p className="text-xs font-semibold truncate">{l.label}</p>
                        <span className={cn(
