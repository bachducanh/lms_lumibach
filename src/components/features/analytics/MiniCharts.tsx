...
export function LineChart({
  ...
  return (
    <svg viewBox={`0 0 ${w} ${height}`} className="w-full h-auto" preserveAspectRatio="none">
      {showAxis && (
        <>
          <line x1={padding} y1={padding} x2={padding} y2={padding + chartH}
            stroke="currentColor" strokeWidth="0.5" className="text-border" />
          <line x1={padding} y1={padding + chartH} x2={padding + chartW} y2={padding + chartH}
            stroke="currentColor" strokeWidth="0.5" className="text-border" />
          <text x={padding - 4} y={padding + 4} textAnchor="end"
            className="fill-muted-foreground text-[9px]">{max}</text>
          <text x={padding - 4} y={padding + chartH + 2} textAnchor="end"
            className="fill-muted-foreground text-[9px]">0</text>
        </>
      )}
      {fill && areaPath && (
      ...
      {points.map((p, i) => (
        <g key={i}>
          <circle cx={p.x} cy={p.y} r={hover === i ? 3.5 : 0}
            fill={color} className="transition-all" />
          <rect
            x={p.x - stepX / 2} y={padding}
            width={Math.max(stepX, 4)} height={chartH}
            fill="transparent"
      ...
      {hover !== null && points[hover] && (
        <g>
          <line x1={points[hover]!.x} y1={padding} x2={points[hover]!.x} y2={padding + chartH}
            stroke={color} strokeDasharray="2 2" strokeWidth="0.8" opacity={0.6} />
          <text x={points[hover]!.x} y={padding - 2} textAnchor="middle"
      ...
      {data.map((d, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text x={padding + i * barW + barW / 2} y={padding + chartH + 12}
              textAnchor="middle" className="fill-muted-foreground text-[9px]">
              {d.label}
            </text>
            {d.value > 0 && (
              <text x={padding + i * barW + barW / 2} y={y - 3}
                textAnchor="middle" className="fill-foreground text-[9px] font-semibold">
                {d.value}
              </text>
            )}
          </g>
