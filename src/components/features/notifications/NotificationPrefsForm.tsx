...
const EMAIL_TOGGLES: { key: PrefKey; label: string; desc: string }[] = [
  ...
  { key: 'emailAssignmentGraded', label: 'Bài tập đã chấm',        desc: 'Khi bài tập được giáo viên chấm và trả' },
  { key: 'emailCodeGraded',       label: 'Bài code đã chấm',       desc: 'Khi bài code exercise được chấm xong' },
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between rounded-lg border border-border p-4">
          <div>
            <Label htmlFor="inApp" className="font-medium">Hiển thị thông báo</Label>
            <p className="text-xs text-muted-foreground mt-0.5">Chuông thông báo trên thanh điều hướng</p>
          </div>
          <Switch
            id="inApp"
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between rounded-lg border border-border p-4">
          <div>
            <Label htmlFor="emailGlobal" className="font-medium">Nhận email thông báo</Label>
            <p className="text-xs text-muted-foreground mt-0.5">Bật/tắt tất cả email thông báo</p>
          </div>
          <Switch
            id="emailGlobal"
        ...
        {prefs.emailEnabled && (
          <div className="space-y-2 pl-1">
            {EMAIL_TOGGLES.map(({ key, label, desc }) => (
              <div key={key} className="flex items-center justify-between rounded-lg border border-border/60 bg-muted/20 px-4 py-3">
                <div>
