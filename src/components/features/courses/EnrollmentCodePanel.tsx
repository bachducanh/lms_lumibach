...
export function EnrollmentCodePanel({ courseId, initialCode, canManage }: Props) {
  ...
  function handleGenerate() {
    startTransition(async () => {
      ...
      if (res.success && res.data) { setCode(res.data.code); toast.success(res.message); }
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4 space-y-2">
      ...
      <p className="text-xs text-muted-foreground">Học sinh dùng mã này để tự đăng ký vào lớp.</p>
    </div>
  );
}
