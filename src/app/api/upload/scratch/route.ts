...
export async function POST(req: NextRequest) {
  ...
  if (!ALLOWED_KINDS.has(kind)) return NextResponse.json({ error: 'Kind không hợp lệ' }, { status: 400 });
  if (!isSb3(file)) return NextResponse.json({ error: 'Cần file Scratch (.sb3)' }, { status: 400 });
  if (file.size > MAX_SIZE) return NextResponse.json({ error: 'File tối đa 50 MB' }, { status: 400 });
