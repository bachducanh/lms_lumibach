  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <Link href={`/courses/${slug}/quizzes`} className="hover:text-foreground transition-colors">Quiz</Link>
        <span className="text-muted-foreground/40">/</span>
        <Link href={`/courses/${slug}/quizzes/${quizId}`} className="hover:text-foreground transition-colors truncate max-w-40">
          {quiz.title}
        </Link>
        <span className="text-muted-foreground/40">/</span>
        ...
        <div>
          ...
          <div className="mt-1 flex flex-wrap gap-4 text-sm text-muted-foreground">
            ...
            {avgScore != null && maxScore != null && (
              <>
                <span>·</span>
                <span>Điểm TB: <span className="font-semibold text-foreground">{avgScore.toFixed(2)}/{maxScore}</span></span>
              </>
            )}
          </div>
        </div>
