...
import {
  listCourseExercisesByModuleAction,
  type CodeExerciseListItem,
} from '@/actions/exercises';
import { hasMinRole } from '@/lib/permissions';
import {
  ClipboardList, Clock, FileText, Paperclip, AlignLeft,
  BookOpen, FolderOpen, Code2,
} from 'lucide-react';
...
function AssignmentCard({ a, slug, isStaff }: { a: AssignmentListItem; slug: string; isStaff: boolean }) {
...
function ExerciseCard({ ex, slug, isStaff }: { ex: CodeExerciseListItem; slug: string; isStaff: boolean }) {
  return (
    <Link
      href={`/courses/${slug}/exercises/${ex.id}`}
      className="flex items-center gap-4 rounded-xl border border-violet-500/20 bg-card px-5 py-4 transition-colors hover:bg-violet-500/5"
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          {isStaff && (
            <span className={cn('shrink-0 rounded-full px-2 py-0.5 text-xs font-medium', EX_STATUS_CLASS[ex.status])}>
...
export default async function AssignmentsPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [{ groups: aGroups, standalone: aStandalone }, { groups: eGroups, standalone: eStandalone }] = await Promise.all([
  ...
  const mergedGroups: MergedGroup[] = allModuleIds.map((moduleId) => {
  ...
  const totalItems =
    ...
    aStandalone.length + eStandalone.length;
    ...
    <div className="max-w-3xl space-y-6">
      ...
      {totalItems === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-muted/30 py-16 text-center gap-2">
          ...
          <p className="font-medium text-muted-foreground">Chưa có bài tập nào</p>
          {isStaff && (
        ...
        <div className="space-y-6">
          ...
          {mergedGroups.map((group) => (
            <div key={group.moduleId} className="space-y-2">
              <div className="flex items-center gap-2.5 rounded-lg bg-primary/5 border border-primary/10 px-4 py-2.5">
                <BookOpen className="h-4 w-4 text-primary shrink-0" />
                <span className="text-sm font-semibold text-primary">{group.moduleName}</span>
