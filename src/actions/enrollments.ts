...
const lookupSelect = {
  id: true, fullName: true, firstName: true, lastName: true, email: true, username: true,
};
...
export async function bulkEnrollAction(
  ...
  for (const raw of identifiers) {
    ...
    if (lookup.kind === 'multiple') {
      errors.push({ identifier, reason: `Trùng tên (${lookup.users.length} người) — dùng email hoặc username` });
      continue;
    }
    ...
    if (existing) { errors.push({ identifier, reason: 'Đã có trong lớp' }); continue; }
...
export async function assignTAAction(
  ...
  if (userId) {
    ...
    const lookup = await findUserByIdentifier(identifier);
    if (lookup.kind === 'notFound') return { success: false, error: `Không tìm thấy: ${identifier}` };
    if (lookup.kind === 'multiple') {
      return { success: false, error: `Trùng tên (${lookup.users.length} người).`, candidates: lookup.users };
    }
  ...
  if (existing) return { success: false, error: `${user.fullName ?? user.email} đã là trợ giảng của lớp này.` };
...
export async function addCoTeacherAction(
  ...
  if (existing) return { success: false, error: `${user.fullName ?? user.email} đã là giáo viên của khoá học này.` };
  ...
  return { success: true, message: `Đã thêm ${user.fullName ?? user.email} làm giáo viên khoá học.` };
}
