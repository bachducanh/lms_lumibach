...
export async function getAdminOverviewAction(): Promise<AdminOverview | null> {
  ...
  const [
    totalUsers, activeUsers7Rows, totalCourses,
    totalQuizAttempts, totalSubmissions, totalCodeSubmissions,
    byRoleRaw,
    dailyActivityRaw, dailyActiveUsersRaw,
    dailySubmissionsRawA, dailySubmissionsRawC,
    topCoursesRaw, topStudentsRaw,
    actionBreakdownRaw,
  ] = await Promise.all([
    prisma.user.count(),
    ...
    prisma.$queryRaw<{ date: string; count: bigint }[]>`
      ...
      GROUP BY 1 ORDER BY 1
    `,
    prisma.$queryRaw<{ id: string; name: string; slug: string; activity: bigint; enrolled: bigint }[]>`
  ...
  const dailySubmissions = Array.from(submissionsMap.entries()).map(([date, count]) => ({ date, count }));
  ...
  return {
    ...
    topCourses: topCoursesRaw.map((c) => ({
      ...
      activity: Number(c.activity), enrolled: Number(c.enrolled),
    })),
    topStudents: topStudentsRaw.map((s) => ({
      id: s.id, name: s.name, activity: Number(s.activity),
    })),
...
export async function getCourseAnalyticsAction(courseSlug: string): Promise<CourseAnalytics | null> {
  const session = await auth();
  ...
  const [
    ...
    prisma.quizAttempt.findMany({
      where:  { quiz: { courseId }, status: 'SUBMITTED', score: { not: null }, maxScore: { not: null } },
    ...
    `,
    prisma.$queryRaw<{
      id: string; title: string; submitted: bigint; avg: number | null; late: bigint;
    }[]>`
      SELECT a.id, a.title,
    ...
    `,
    prisma.$queryRaw<{
      id: string; title: string; attempts: bigint; submitted: bigint; avg: number | null;
    }[]>`
      SELECT q.id, q.title,
    ...
    `,
    prisma.$queryRaw<{
      id: string; title: string; students: bigint; passers: bigint;
    }[]>`
  ...
  if (quizAttemptStats.length > 0) {
    ...
    if (valid.length > 0) {
      avgQuizPercent = valid.reduce((acc, qa) => acc + (qa.score! / qa.maxScore!) * 100, 0) / valid.length;
    }
  }
  ...
  return {
    ...
    inactiveStudents: inactiveRaw.map((r) => ({
      id: r.id, name: r.name, lastSeenAt: r.lastSeenAt,
    })),
