...
export async function createScratchExerciseAction(input: {
  ...
  const course = await prisma.course.findUnique({ where: { id: input.courseId }, select: { ownerId: true } });
...
export async function submitScratchAction(input: {
  ...
  logActivity({
    userId, courseId: ex.courseId, action: 'SUBMIT_CODE',
    resourceType: 'exercise', resourceId: ex.id, resourceName: ex.title,
  });
...
export async function gradeScratchSubmissionAction(input: {
  ...
  const sub = await prisma.codeSubmission.findUnique({
    ...
    select: { id: true, codeExercise: { select: { courseId: true, course: { select: { ownerId: true } } } } },
  });
...
export async function listMyScratchSubmissionsAction(exerciseId: string) {
  ...
  return prisma.codeSubmission.findMany({
    ...
    orderBy: { attemptNumber: 'desc' },
    select:  {
      id: true, status: true, score: true, maxScore: true, feedback: true,
      submittedAt: true, attemptNumber: true, code: true, gradedAt: true,
    },
  });
}
...
export async function listScratchSubmissionsAction(exerciseId: string) {
  ...
  const subs = await prisma.codeSubmission.findMany({
    ...
    select:  {
      ...
      feedback: true, submittedAt: true, attemptNumber: true, code: true, gradedAt: true,
    },
  });
  ...
  return subs.map((s) => ({
    ...
    student: byId.get(s.studentId) ?? {
      id: s.studentId, fullName: null, firstName: '?', lastName: '?', avatar: null,
    },
  }));
}
