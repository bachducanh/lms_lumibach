...
export async function createLessonAction(
  ...
  const lesson = await prisma.lesson.create({
    data: { title, content, estimatedMinutes: estimatedMinutes ?? null, createdBy: session!.user!.id },
  });
  ...
  return { success: true, message: 'Đã tạo bài giảng.', data: { lessonId: lesson.id, itemId: item.id } };
}
