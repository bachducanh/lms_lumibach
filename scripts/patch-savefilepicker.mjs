...
const OLD = "showSaveFilePicker: typeof showSaveFilePicker === 'function' && !navigator.userAgent.includes('Android') ? window.showSaveFilePicker.bind(window) : null";
...
for (const file of BUNDLES) {
  ...
  if (!c.includes(OLD)) {
    console.log('  - ' + file.split(/[\\/]/).pop() + ' (no SB3Downloader.defaultProps — ok to skip)');
    continue;
  }
