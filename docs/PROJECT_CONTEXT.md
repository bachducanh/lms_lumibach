...
> **Kiến trúc đang migrate (QĐ-008):** Từ Next.js fullstack → tách FE (Next.js) + BE (NestJS) trong monorepo pnpm workspace. Stack dưới đây phản ánh kiến trúc TARGET sau migration. Trong giai đoạn middle migrate, một số module vẫn còn ở Next.js Server Actions cho đến khi được migrate sang NestJS. Xem [MIGRATION_PLAN.md](MIGRATION_PLAN.md) để biết tiến độ.

### Frontend (`apps/web`)

- **Framework**: Next.js 16 với App Router (Server Components fetch BE qua HTTP)
- **Ngôn ngữ**: TypeScript (strict mode, `noUncheckedIndexedAccess`)
- **Styling**: Tailwind CSS 4
- **UI Components**: shadcn/ui (dựa trên `@base-ui/react`)
- **Form management**: react-hook-form + Zod validation (Zod schema import từ `packages/types`)
- **State management**: React Context + Server Components (KHÔNG dùng Redux)
- **Data fetching**: HTTP client tự viết (`apps/web/src/lib/api-client.ts`) wrapping `fetch`; Server Components dùng `fetch()` server-side với cache tag
- **WebSocket**: socket.io-client cho real-time (notification, code result, quiz timer)
- **Rich text editor**: TipTap v3 (extensible, headless)
...
- **ORM**: Prisma 5+ (share qua `packages/db`)
- **Authentication**:
  - Giai đoạn middle migrate: verify NextAuth JWT bằng shared `AUTH_SECRET` (auth bridge)
  - Sau Phase 7: JWT do NestJS phát hành (`@nestjs/jwt` + `@nestjs/passport` + `passport-jwt`), bcrypt cost 12, refresh token rotation, dual mode cookie/JSON cho web+mobile
- **Validation**: `nestjs-zod` — share Zod schema với FE từ `packages/types` (single source of truth)
- **API docs**: `@nestjs/swagger` auto OpenAPI tại `/api/docs`
- **API versioning**: URI versioning từ Phase 1 (`/api/v1/...`)
- **File storage**: MinIO self-hosted (S3-compatible) — BE stream upload qua `@nestjs/platform-express` + multer; FE/mobile load qua Nginx proxy cùng domain
- **Cache**: Redis 7 + `@nestjs/cache-manager` + `cache-manager-redis-yet` — cache-aside cho endpoint đọc nhiều
- **Queue**: BullMQ + `@nestjs/bullmq` — producer ở `apps/api`, consumer ở `apps/worker` (process riêng để scale độc lập)
- **Cron**: `@nestjs/schedule` thay external HTTP cron
- **WebSocket**: `@nestjs/websockets` + `@nestjs/platform-socket.io` — NotificationGateway, CodeExecutionGateway, QuizGateway
- **Rate limit**: `@nestjs/throttler` — login 5/15p/IP, code submit 10/min/user, API 100/min/user
- **Email**: Nodemailer SMTP, gửi qua BullMQ worker
- **Logging**: `nestjs-pino` (structured JSON, redact PII)
- **Error tracking**: `@sentry/node`

### Shared packages

- **`packages/db`**: Prisma schema + generated client singleton. Owner duy nhất của migration.
- **`packages/types`**: Zod schemas + TypeScript types + enums share giữa FE/BE. Single source of truth.
- **`packages/config`**: ESLint, TypeScript, Prettier configs share toàn workspace.

### Background workers (`apps/worker`)

- Process riêng (không expose HTTP), load các `@Processor` của BullMQ
- Queues: `code-execution` (Judge0), `email`, `notification-fanout`
- Share `packages/db` + `packages/types` với `apps/api`
- Có thể scale độc lập (chạy nhiều instance khi cần)
...
- **TypeScript everywhere**: type safety giảm bug, AI hỗ trợ tốt hơn, share type giữa FE/BE qua `packages/types`
- **Next.js cho FE**: App Router + Server Components giúp SEO tốt, streaming UI, ít JS bundle
- **NestJS cho BE**: Tách FE/BE để scale độc lập, test dễ (dependency injection), WebSocket built-in tốt, OpenAPI auto, hệ sinh thái Bull/Schedule/Throttler sẵn — giảm boilerplate đáng kể so với Express thuần
- **Monorepo pnpm workspace**: 1 repo, share Prisma client + Zod schema giữa FE/BE, không phải maintain 2 nguồn type — vẫn lý tưởng cho solo dev
- **PostgreSQL**: Chuẩn công nghiệp, mạnh, miễn phí
- **Prisma**: Type-safe ORM, migration tự động, schema rõ ràng — tiết kiệm hàng trăm giờ debug
- **Zod single source**: 1 schema dùng cho cả `react-hook-form` resolver (FE) và `nestjs-zod` validation pipe (BE) — không trùng nguồn validation
- **Redis cache layer**: Cải thiện hiệu năng real, bù lại overhead HTTP hop sau khi tách FE/BE
...
**Tách Frontend/Backend, mỗi side là Modular Monolith** (QĐ-008). Cụ thể:

- `apps/web` — Next.js FE, modular theo feature trong `src/components/features/`
- `apps/api` — NestJS BE, modular theo domain trong `src/modules/`
- `apps/worker` — Background process riêng (BullMQ consumers)
- Judge0 + MinIO + PostgreSQL + Redis — services độc lập

Lý do không chọn microservices (chia nhỏ BE thành nhiều service): Solo developer + 600 user → microservices tạo phức tạp không cần thiết. Modular monolith ở mỗi side đủ để scale ngang khi cần, tách module thành service riêng được khi thực sự cần.

### Sơ đồ thành phần hệ thống (target sau migration QĐ-008)
...
```
              ...
              │ Server Compo-  ││   │ └──────┘ └──────────┘ │
              │ nents fetch    │┼──▶│ ┌──────┐ ┌──────────┐ │
              │ apps/api qua   ││   │ │ Quiz │ │ Gradebook│ │
              │ HTTP nội bộ    ││   │ └──────┘ └──────────┘ │
              │                ││   │ ┌──────┐ ┌──────────┐ │
              │ Client Compo-  ││   │ │ Code │ │WebSocket │ │
              │ nents qua HTTP ││   │ │ Exec │ │ Gateway  │ │
              │ + Socket.IO    ││   │ └──┬───┘ └──────────┘ │
              └────────────────┘│   └────┼──┬──┬───────────┘
                       │        │        │  │  │
                       │        │        │  │  └──── BullMQ enqueue ─┐
                       ...
                       ▼        ▼        ▼  ▼                         ▼
              ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌─────────┐ ┌──────────┐
              │PostgreSQL│ │  Redis   │ │  MinIO   │ │ Judge0  │ │apps/worker│
              │  (DB)    │ │(Cache+Q) │ │ (Storage)│ │(CodeExe)│ │(Consumers)│
              └──────────┘ └──────────┘ └──────────┘ └────┬────┘ └─────┬────┘
                                                          │            │
                                                          ▼            │
                                                  ┌───────────────┐    │
...
Shared packages (build-time, không chạy runtime):
  - packages/db    → Prisma client + migrations
  - packages/types → Zod schemas + DTOs (FE và BE cùng import)
  - packages/config → TS / ESLint / Prettier configs
```

**Lưu lượng giao tiếp:**

- Browser → Nginx → web (port 3000 nội bộ) cho HTML / RSC
- Browser → Nginx → api (port 4000 nội bộ) cho REST `/api/v1/*` và WebSocket `/ws/*`
- Browser → Nginx → MinIO (port 9000 nội bộ) cho file `/files/*` (signed URL)
- Server Component của web → api (qua Docker network nội bộ, không qua public internet)
- api → worker: thông qua Redis BullMQ (không gọi trực tiếp)
- worker → Judge0: qua Docker network, container name `lumibach-judge0`
...
### Data flow điển hình - Học sinh nộp bài code (sau migration QĐ-008)

```
1. Học sinh viết code trong Monaco Editor (apps/web - Client Component)
2. Click Submit → apiClient.post('/api/v1/submissions') → REST tới apps/api
3. NestJS Controller: AuthGuard verify cookie → ZodValidationPipe validate body
4. SubmissionService: lưu submission vào PostgreSQL với status "pending"
5. SubmissionService: enqueue job vào BullMQ queue 'code-execution' (Redis)
6. Controller trả response ngay: { success: true, data: { submissionId } }
7. apps/web Client Component: join WebSocket room 'submission:${submissionId}'
8. apps/worker (process riêng): pick job từ queue
9. Worker gửi code + test cases tới Judge0 qua Docker network nội bộ
10. Judge0 chạy code trong Docker sandbox isolated
11. Judge0 trả kết quả (pass/fail từng test case + output)
12. Worker update submission trong PostgreSQL với kết quả
13. Worker publish event qua Redis Pub/Sub → CodeExecutionGateway của apps/api
14. Gateway emit 'submission.complete' tới room 'submission:${submissionId}'
15. apps/web Client nhận event realtime, hiển thị kết quả cho học sinh
```

**Tại sao tách worker process riêng:** Code execution CPU-intensive + có thể bị abuse (fork bomb, infinite loop). Tách `apps/worker` cho phép giới hạn resource process này riêng (cgroups), không ảnh hưởng tới khả năng response của `apps/api` cho các user khác.
...
### QĐ-008: Tách Backend thành NestJS, chuyển sang monorepo

**Ngày**: 2026-05-11
**Quyết định**: Tách backend ra thành service NestJS độc lập (`apps/api`), giữ Next.js làm FE thuần (`apps/web`). Tổ chức repo theo pnpm workspace monorepo với các package share (`packages/db` cho Prisma, `packages/types` cho Zod schemas).

**Lý do**:

- Hiệu năng Next.js Server Actions không tối ưu được như mong muốn (đặc biệt khi 100+ học sinh đồng thời làm quiz/code).
- Khó scale BE độc lập với FE: code execution heavy + DB query nặng đang block React render.
- Khó test BE vì server actions gắn chặt với React component context, không inject được dependency.
- Cần REST API cho mobile app sau này (smartphone Android là thiết bị chính của nhiều học sinh).
- Code organization: 21 server actions rải rác ở `src/actions/` khó refactor, khó phân quyền tập trung.
- Giai đoạn middle migrate cần WebSocket real-time cho notification + code submission result + quiz timer authoritative (NestJS hỗ trợ tốt hơn Next.js).

**Trade-off**:

- Thêm 1 service phải maintain (mitigation: tái dùng infrastructure đã có — Redis, BullMQ, Prisma).
- Thêm 1 HTTP hop FE→BE (mitigation: Redis cache, batch endpoints, BE-FE cùng Docker network nội bộ).
- Thời gian migrate 4-6 tháng part-time, không có feature mới trong giai đoạn này (chấp nhận được vì có Moodle backup).

**Strategy**: Strangler Fig Pattern theo 11 phase, mỗi phase 1 PR branch. Auth migration đặt CUỐI (Phase 7) để giảm rủi ro. Phase 0.5 đo performance baseline trước khi migrate để có số so sánh.

**Phản đảo QĐ-001 (Modular Monolith)?**: Không. Vẫn modular monolith trong từng app (`apps/web` và `apps/api`). Chỉ tách BE thành service riêng vì lý do trên — tương tự cách Judge0 và MinIO đã được tách từ đầu trong QĐ-001.

**Chi tiết kế hoạch triển khai**: Xem [MIGRATION_PLAN.md](MIGRATION_PLAN.md).
