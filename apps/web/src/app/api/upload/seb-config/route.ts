...
export async function POST(req: NextRequest) {
  ...
  if (!file.name.toLowerCase().endsWith('.seb')) {
    return NextResponse.json(
      { error: 'Chỉ hỗ trợ file cấu hình Safe Exam Browser (.seb)' },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    const objectName = `seb-configs/${courseId}/${randomBytes(10).toString('hex')}.seb`;
    const buffer = Buffer.from(await file.arrayBuffer());

    await minioClient.putObject(BUCKET_FILES, objectName, buffer, buffer.length, {
      // SEB nhận diện file cấu hình qua MIME này; trình duyệt sẽ kích hoạt SEB khi mở.
    ...
    console.error('[SEB CONFIG UPLOAD]', err);
