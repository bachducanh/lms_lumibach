...
const ENROLLMENT_STATUS_CLASS: Record<string, string> = {
  ...
  COMPLETED:
    'border border-blue-600/25 bg-blue-50 text-blue-800 dark:border-blue-500/30 dark:bg-blue-500/10 dark:text-blue-400',
  ...
  return (
    <div className="space-y-4">
      ...
      {enrollments.length === 0 ? (
        <div className="border-border bg-card rounded-xl border border-dashed px-4 py-10 text-center">
        ...
        <div className="space-y-3">
          {enrollments.map((e) => (
            <div
              key={e.id}
              className="border-border bg-card space-y-3 rounded-xl border px-4 py-4 shadow-sm"
            >
              {/* Course name + status */}
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div className="min-w-0">
                  <Link
                    ...
                    className="hover:text-primary font-semibold break-words transition-colors"
                  >
                    {e.courseName}
                  </Link>
                  <span
                ...
                {canManage && (
                  <button
                    ...
                    className="border-destructive/30 text-destructive hover:bg-destructive/10 inline-flex h-9 items-center gap-1.5 rounded-full border px-3.5 text-sm transition-colors disabled:opacity-50"
                  >
                    {removingId === e.id ? (
              ...
              <div className="flex flex-wrap gap-4 text-sm">
                {e.quizScore != null && (
                  <div className="flex items-center gap-1.5">
                    <span className="text-muted-foreground text-sm">Quiz:</span>
                    <span
                      className={cn(
