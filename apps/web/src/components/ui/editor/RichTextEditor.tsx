  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div
          className={cn(
            'border-border bg-background absolute top-full z-50 mt-1 rounded-lg border shadow-lg',
...
function ColorPicker({
  ...
  return (
        ...
        >
          ...
          <div
            ...
            style={{
              ...
              border: activeColor ? 'none' : '1px dashed currentColor',
            }}
          />
        </button>
      )}
    >
      {(close) => (
        <div className="grid w-36 grid-cols-4 gap-1 p-2">
          {colors.map((c) => (
            <button
              key={c.value ?? 'none'}
              type="button"
              ...
              className={cn(
                ...
                c.value === null && 'text-muted-foreground flex items-center justify-center text-xs'
              )}
...
function MenuLabel({ children }: { children: React.ReactNode }) {
  return (
    <div className="text-muted-foreground px-3 py-0.5 text-xs font-semibold tracking-wide uppercase">
      {children}
    </div>
  );
}
  ...
  const editor = useEditor({
    extensions: [
      StarterKit.configure({
        ...
        codeBlock: {
          HTMLAttributes: { class: 'not-prose rounded-md bg-muted p-4 font-mono text-sm' },
        },
        link: {
          openOnClick: false,
  ...
  return (
    <div className={cn('border-input bg-background rounded-xl border shadow-sm', className)}>
      ...
      <div className="border-input bg-background rounded-t-xl border-b">
        ...
        {/* Row 2 */}
        <div className="border-input/60 flex flex-wrap items-center gap-0.5 border-t px-2 py-1.5">
          {/* Lists */}
          <TBtn
