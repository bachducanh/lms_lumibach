...
import type { CodeLanguage, CodeSubmissionStatus, ExerciseStatus, UserRole } from '@lumibach/db';
...
export async function createExerciseAction(courseId: string, data: CreateExerciseInput) {
  ...
  if (!userId || !role || !hasMinRole(role, 'TA'))
    return { success: false as const, error: 'Không có quyền' };
...
export async function saveExerciseTestCasesAction(
  exerciseId: string,
  testCases: TCInput[]
): Promise<ActionResult> {
  const session = await auth();
