...
import type { UserRole, AssignmentStatus } from '@lumibach/db';
...
export async function listAssignmentsByModuleAction(
  courseId: string
): Promise<AssignmentsByModule> {
  const session = await auth();
...
export async function createAssignmentAction(
  ...
  if (!(await canManageCourse(session.user.id, session.user.role as UserRole, courseId)))
  ...
  return {
    success: true,
    message: publish ? 'Đã đăng bài tập.' : 'Đã lưu nháp.',
...
export async function updateAssignmentAction(
  ...
  if (!(await canManageCourse(session.user.id, session.user.role as UserRole, existing.courseId)))
    return { success: false, error: 'Không có quyền.' };
...
export async function submitAssignmentAction(
  ...
  if (assignment.status !== 'PUBLISHED')
    return { success: false, error: 'Bài tập chưa được đăng.' };
