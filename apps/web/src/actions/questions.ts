...
export async function updateCategoryAction(
  ...
  const cat = await (prisma as any).questionCategory.findUnique({
    where: { id: categoryId },
    select: { courseId: true },
  });
  if (!cat) return { success: false, error: 'Không tìm thấy danh mục.' };
  if (!(await canManageCourse(session.user.id, role, cat.courseId)))
    return { success: false, error: 'Không có quyền.' };
...
export async function runSolutionCodeForTCAction(
  ...
  try {
    ...
    if (r.status.id === 6)
      return { success: false, error: `Lỗi compile:\n${r.compile_output ?? ''}` };
