...
import type { UserRole, QuizStatus } from '@lumibach/db';
...
export async function listQuizBanksAction(courseId: string): Promise<QuizBank[]> {
  ...
  if (uncategorized.length > 0) {
    result.push({
      id: '__none',
      title: 'Chưa phân danh mục',
      questions: uncategorized as QuestionBankItem[],
    });
  }
...
export async function createQuizAction(
  ...
  const role = session.user.role as UserRole;
  if (!(await canManageCourse(session.user.id, role, courseId)))
  ...
  return {
    success: true,
    message: publish ? 'Đã đăng quiz.' : 'Đã lưu nháp.',
    data: { quizId: quiz.id },
  };
}
...
export async function updateQuizAction(
  ...
  if (!(await canManageCourse(session.user.id, role, existing.courseId)))
    return { success: false, error: 'Không có quyền.' };
...
export async function addQuestionToQuizAction(
  ...
  if (!(await canManageCourse(session.user.id, role, quiz.courseId)))
    return { success: false, error: 'Không có quyền.' };
...
export async function setQuizQuestionPointsAction(
  quizId: string,
  questionId: string,
  points: number | null
): Promise<ActionResult> {
...
export async function addMultipleQuestionsToQuizAction(
  quizId: string,
  questionIds: string[]
): Promise<ActionResult> {
  const session = await auth();
...
export async function addRandomQuestionsToQuizAction(
  ...
  if (pool.length === 0)
    return { success: false, error: 'Không còn câu hỏi nào trong danh mục này.' };
